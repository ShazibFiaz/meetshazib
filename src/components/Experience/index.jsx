...
export default function ClientExperienceAndEducationView({
  ...
  return (
    <div
      className="max-w-screen-xl mt-24 mb-6 sm:mt-14 sm:mb-14 px-4 sm:px-8 mx-auto"
      id="experience"
    >
              ...
              <h1 className="mb-4 text-3xl lg:text-4xl xl:text-5xl flex items-center gap-3 xl:ml-4">
                ...
                {"Experience".split(" ").map((item, index) => (
                  <span
                    key={index}
                    className={`${
                      index === 1 ? "text-green-main" : "text-white px-5"
                    }`}
                  >
                    {item}{" "}
                  </span>
              ...
              <VerticalTimeline layout="1-column-left" lineColor="#ccc">
                {experienceData && experienceData.length
                  ? experienceData.map((experienceItem, index) => (
                      ...
                      >
                        <h3 className="text-amber-400 text-2xl py-1 mt-[-65px]">
                          {experienceItem.position}
                        </h3>
                        <h3 className="text-xl text-gray-400 py-1">
                        ...
                        <div className="mt-4">
                          {experienceItem.jobProfile.map((task, taskIndex) => (
                            <p
                              key={taskIndex}
                              className="text-lg text-gray-400 mb-2 leading-relaxed"
                            >
                              • {task}
                            </p>
                          ))}
          ...
          <AnimationWrapper className="py-6 sm:py-16">
            <div className="flex flex-col justify-center sm:items-start items-center text-center sm:text-right">
              <h1 className="mb-4 text-3xl lg:text-4xl xl:text-5xl flex items-center gap-3 xl:ml-3">
                ...
                {"Education/Certifications".split(" ").map((item, index) => (
                  <span
                    key={index}
                    className={`${
                      index === 1 ? "text-green-main" : "text-white px-5"
                ...
                {educationData && educationData.length
                  ? educationData.map((educationItem, index) => (
                      ...
                      >
                        ...
                        <h4 className="text-xl mt-[10px] text-gray-400">
                          {educationItem.year}
                        </h4>
                        <h4 className="text-xl py-1 mt-[10px]">
